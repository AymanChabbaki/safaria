...
const AdminCaravanesPage = () => {
  ...
  const handleSubmitAdd = async (e) => {
    e.preventDefault();
    try {
      await api.caravanes.create(formData);
  ...
  const handleSubmitEdit = async (e) => {
    e.preventDefault();
    try {
      await api.caravanes.update(selectedCaravane.id, formData);
            ...
            <tbody className="divide-y divide-gray-200">
              {caravanes.map((caravane, idx) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 font-medium text-gray-800">{caravane.name}</td>
                  <td className="px-6 py-4 text-gray-600 max-w-md truncate">{caravane.description}</td>
          ...
          >
            ...
            >
              ...
              <div className="p-6 space-y-6">
                ...
                {/* Image Gallery */}
                {selectedCaravane.images && selectedCaravane.images.length > 0 && (
                  <div>
                    <h4 className="text-sm font-semibold text-gray-700 mb-3">Galerie d'images</h4>
                    <div className="grid grid-cols-3 gap-3">
                      {selectedCaravane.images.map((img, idx) => (
                        <div key={idx} className="aspect-square rounded-lg overflow-hidden">
                          <img
                            src={`http://localhost:5000${img}`}
                            alt={`${selectedCaravane.name} ${idx + 1}`}
                            className="w-full h-full object-cover"
                          />
                        </div>
                      ))}
                ...
                <div>
                  <h3 className="text-xl font-bold text-gray-800 mb-2">{selectedCaravane.name}</h3>
                  <p className="text-gray-600">{selectedCaravane.description}</p>
                </div>

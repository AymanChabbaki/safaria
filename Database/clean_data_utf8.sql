-- ============================================================
-- Clean Re-Insert of All Data with Proper UTF-8 Encoding
-- ============================================================
...
('Ferronnerie d\'Art Essaouira', 
 'Explorez notre atelier de ferronnerie d\'art dans la médina d\'Essaouira où sont créées des pièces uniques: portes sculptées, lanternes ornées, mobilier et objets décoratifs en fer forgé traditionnel. Démonstration du travail du forgeron, possibilité de commandes personnalisées, cours d\'initiation à la forge, exportation possible dans le monde entier. Ouvert du mardi au dimanche de 9h à 17h. Contact: +212 5 24 47 56 78', 
 'artisanat',
 31.5125, -9.7595,
 180.00,
...
-- Insert Sejours with proper French characters
INSERT INTO sejours (name, description, category, latitude, longitude, price, main_image) VALUES
('Riad Traditionnel Marrakech', 
 'Séjour authentique dans un magnifique riad du 18ème siècle entièrement restauré au cœur de la médina de Marrakech. Patio avec fontaine centrale, terrasse panoramique avec vue sur les toits de la ville rouge, décoration traditionnelle raffinée avec zellige et bois sculpté. Piscine sur la terrasse, WiFi gratuit, petit-déjeuner marocain inclus, hammam traditionnel, salon berbère, cuisine traditionnelle sur demande. Capacité maximale: 8 personnes. Prix: 450 DH par nuit. Ouvert toute l\'année. Contact: +212 5 24 38 12 34', 
 'sejour',
 31.6295, -7.9811,
 450.00,
...
('Circuit Atlas et Désert', 
 'Circuit complet de 7 jours combinant les montagnes de l\'Atlas et le désert du Sahara. Randonnées dans les vallées verdoyantes de l\'Atlas, rencontres authentiques avec des familles berbères, nuits en kasbah traditionnelle et bivouac sous les étoiles dans l\'Erg Chebbi. Transport en 4x4 tout confort climatisé, guide professionnel francophone diplômé, hébergement varié (hôtels de charme, kasbahs, bivouac), tous les repas inclus avec menu varié, méharée dans le désert (2 jours), visite de villages berbères de l\'Atlas. Durée: 7 jours / 6 nuits. Groupe: 8 personnes max. Prix: 890 DH par personne. Ouvert toute l\'année. Départ: Marrakech. Contact: +212 5 24 38 99 88', 
 'caravane',
 31.2000, -7.0000,
 890.00,
...
SELECT a.id, a.name, 'artisanat' as type FROM artisans a 
JOIN images_360 i ON i.itemType = 'artisanat' AND i.itemId = a.id
UNION
SELECT s.id, s.name, 'sejour' as type FROM sejours s 
JOIN images_360 i ON i.itemType = 'sejour' AND i.itemId = s.id
UNION
SELECT c.id, c.name, 'caravane' as type FROM caravanes c 
